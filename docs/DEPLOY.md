...
- На сервере должен быть файл **`.env`** (в корне проекта или в `bot/`). Скопировать с `bot/.env.example`, заполнить:
  ...
  - **BOT_TELEGRAM_PROXY** (опционально) — прокси для доступа к `api.telegram.org` (если сеть не доходит до Telegram: блокировки, фильтры). Формат URL: `socks5://логин:пароль@хост:порт` или `socks5://хост:1080`. Нужен рабочий SOCKS5/HTTP-прокси (VPS за границей, коммерческий прокси и т.п.). После изменения — `uv sync` и перезапуск бота.
