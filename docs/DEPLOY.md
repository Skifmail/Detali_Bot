...
- На сервере должен быть файл **`.env`** (в корне проекта или в `bot/`). Скопировать с `bot/.env.example`, заполнить:
  ...
  - **BOT_LOG_PATH** (опционально) — путь к файлу логов (например `/var/log/detali-bot/bot.log`). Если задан, логи дополнительно пишутся в файл с ротацией (1 MB, хранится 3 файла). Иначе логи только в stdout (терминал / journald). В админ-меню (**Ещё → 📊 Статус бота**) хвост лога берётся из этого файла, если он существует и читается процессом; иначе — из `journalctl` для unit (см. ниже).
  - **BOT_SYSTEMD_UNIT** (опционально) — имя systemd unit для `systemctl` / `journalctl` (по умолчанию `detali-bot.service`). Задайте, если unit на сервере называется иначе.
  - **Прокси для Telegram Bot API** (опционально), если с сервера не открывается `api.telegram.org`:
    - **Вариант A (рекомендуется при пароле со спецсимволами):** `BOT_TELEGRAM_PROXY_HOST`, `BOT_TELEGRAM_PROXY_PORT` (по умолчанию 1080), при необходимости `BOT_TELEGRAM_PROXY_USER` и `BOT_TELEGRAM_PROXY_PASSWORD` — логин/пароль передаются отдельно, без вставки в URL.
    - **Вариант B:** одна строка `BOT_TELEGRAM_PROXY` — например `socks5://логин:пароль@хост:1080` или `socks5://хост:1080` без авторизации. Если в пароле есть `@`, `:`, `#` и т.п., URL может разобраться неверно — используйте вариант A.
    - Если задан `BOT_TELEGRAM_PROXY_HOST`, строка `BOT_TELEGRAM_PROXY` не используется.
    - После изменения: `uv sync` (если обновляли зависимости) и перезапуск бота.
  - **Ошибка `Username and password authentication failure` / `ProxyError`:** прокси отклонил логин или пароль — проверьте учётные данные у провайдера прокси; при доступе по IP без логина уберите `BOT_TELEGRAM_PROXY_USER` и `BOT_TELEGRAM_PROXY_PASSWORD` (или оставьте только `socks5://хост:порт` в `BOT_TELEGRAM_PROXY`).
